...
We perform the one-sample test for the trend sample of $\beta_{gi},$ and
$\pi_0$ equal to the neutral element (uniform density on $(a,b).$ it
allows to answer the question: "are the mean trend densities in the
provinces equal to the Bayes neutral element $\pi_0$" ?

```{r tbl-one_sample}
n <- length(vnt$t_max)
vnt_pca <- pca.fd(c(vnt$t_max), nharm = 12)
lambda <- vnt_pca$values
scores <- vnt_pca$scores
v <- vnt_pca$harmonics

# PC test
tpc <- n * sum(inprod(v[1:3], vnt_mean)^2 / lambda[1:3])
tpc_pval <- pchisq(tpc, 3, lower.tail = FALSE)

# Norm test
tnorm <- n * sum(inprod(v[lambda[1:12] > 0], vnt_mean)^2)
tnorm_pval <- imhof(tnorm, lambda[lambda[1:12] > 0])[[1]]
...
kable_format(one_sample_tbl)
```

## Distributional ANOVA


```{r fig-trends_regional_means}
#| fig-cap: Regional mean trend and overall trend.
vntr <- vnt |>
  select(!province) |>
...
apply for each province the trend estimation and obtain the set of trend
densities $\beta_{gi=}.$ Then we test whether these trend densities vary
